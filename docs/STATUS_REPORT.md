...
METIS is a full-stack AI-powered business management platform built with Next.js (frontend), FastAPI (backend), Google Gemini AI, and Firestore. The project has a well-structured monorepo with clean separation of concerns. The core backend infrastructure (Milestones 0-4) and all frontend pages (Milestone 6) are complete. **All previously reported critical frontend-backend integration gaps are now resolved**: business setup persists to the backend, the business ID survives page reloads, the API base URL is environment-configurable, and every frontend page now calls the real API with loading and error states. **Chat conversations are persisted with multi-turn Gemini context and a history endpoint** — the Business Chat survives page reloads and the Manager Agent remembers prior turns. **METIS 0.3.0 (2026-08-12)** adds local **SQLite persistence** (`backend/data/metis.db` — data survives restarts without Google Cloud), **in-app Gemini API key management** in the Chat page, and fixes **approval execution failures** (staged approvals no longer fail on truncated/hallucinated IDs, and the Approval Center surfaces the real execution error instead of a generic message). **METIS 0.4.1 (2026-08-17)** turns the Business Chat into a catalog-management surface: the Operations Agent gained `create_product`/`delete_product` (staged into the Approval Center), Products gained an optional unique `product_key` (SKU) enforced by the API (409) and the agents, product PUT/DELETE now enforce existence and ownership (they previously wrote/deleted blindly), the Products UI was overhauled (live search, edit mode, delete with confirmation, product-key chips), and a frontend favicon set was added. **METIS 0.4.2** adds the `set_stock` inventory-override tool, which executes immediately — the owner can say "mark Deadpool's Golden Glock out of stock" and it happens on the spot instead of the agent reporting it can't. **METIS 0.4.3** makes the backend serve the favicon the browser requests (`/favicon.ico` + `/icon.svg` routes in `backend/src/main.py`, static copies under `backend/src/static/`, hidden from the `/docs` schema). **METIS 0.5.0 (2026-08-17, Phase 0 baseline sync)** reconciles the docs with the codebase: the **customer-facing storefront chat is already implemented** (public `frontend/src/app/storefront/[businessId]/page.tsx` + `GET/POST /api/storefront/{business_id}/history|chat`, commit `04645a0` 2026-08-12), so Milestone 7's step 2 is complete and the integration-gap table is corrected. Version marker bumped to 0.5.0 everywhere. **Phase 1 (E2E verification) done**: `backend/scripts/e2e_demo.py` (committed) passes **27/27** against a clean SQLite DB — REST CRUD, `product_key` 409, cross-business 404, server-side totals, inventory decrement, revenue booking, analytics, plus **live Gemini flows** (owner chat stages `create_product` → approve → catalog updated; storefront chat stages `create_order` → approve → stock 8→6). Supporting changes: `METIS_DB_PATH` env override for isolated test DBs. **Phase 1B started (demo experience)**: `POST /api/demo/seed` + Setup Wizard "Load the demo store" button (5 products, 3 customers, 3 revenue-bearing orders); Orders page now shows the customer name under the `customer #XXXX` tag; new **order memo PDF** (`/orders/{orderId}/receipt`, print-to-PDF, zero new deps). **METIS 0.6.0 (2026-09-04, Phase 1B second batch)** finishes most of the remaining demo-experience work: **mock AI mode** (`METIS_MOCK_AI=1`) answers common owner-chat intents deterministically with no Gemini key, by pattern-matching the message and dispatching to the same tool the real model would call; **SSE-streamed chat** for both the owner and storefront chat endpoints (`POST /api/chat/{business_id}/stream`, `POST /api/storefront/{business_id}/chat/stream`) — the reply is computed the normal way then replayed to the client word-by-word; and **photo→product drafting** (`POST /api/products/{business_id}/from-photo`) which asks Gemini vision for a JSON draft the owner reviews before saving. **METIS 0.7.0 (2026-09-04, Milestone 9)** adds a committed pytest suite — `backend/tests/` (58 tests) — covering agent permissions, product/customer/order CRUD and ownership guards, order booking/release/reapply, revenue recognition, the full approval stage→approve/reject→execute pipeline including execution failures, mock-AI chat tool dispatch, and demo seeding; every test runs against an isolated temp SQLite DB with mock AI mode on, no Gemini key required. Writing the suite surfaced and fixed a real bug: `GET /analytics/{business_id}/revenue` never actually accepted the documented `period` query param. **METIS 0.7.1 (2026-09-04)** adds per-business currency: `Business.currency` (default `BDT`), picked once in the Setup Wizard from a curated list served at `GET /api/currencies`, replacing ~20 hardcoded Taka-symbol occurrences across backend agent prompts and the frontend's shared `<Cash>` component. **METIS 0.7.2 (2026-09-04)** finishes Phase 1B with voice briefing: `GET /api/agents/{business_id}/briefing` plus a Dashboard button that reads the Manager Agent's daily summary aloud via the browser's `SpeechSynthesis` API (zero new deps); `produce_summary()` builds a deterministic sentence from live metrics under mock AI mode so it works with no Gemini key. **METIS 0.7.5 (2026-09-04)** makes the data layer safe and economical for more than one tenant: `list_all()` stopped streaming whole collections and filtering in Python — filters are pushed into the query (`FieldFilter` on Firestore, a `json_extract` WHERE clause on SQLite behind a new expression index), so listing one shop's products no longer reads every other tenant's documents; and `require_business_access` (`backend/src/core/auth.py`) guards all 28 owner routes, with `Business.owner_uid` stamped from the authenticated caller rather than the request body. **METIS 0.7.6 (2026-09-04)** supplies the identity half of Milestone 8 without changing a single route signature: an HTTP middleware resolves the bearer token through the thin `verify_token` seam in `backend/src/core/firebase.py` and sets `request.state.uid`, which `get_current_uid` had been reading since 0.7.5. It decodes and continues rather than rejecting, because the public storefront must stay open to shoppers with no account. The frontend gained `AuthContext`, a `/login` page, and one guard in `AppShell.tsx` inherited by all nine owner pages, while `GET /api/businesses` replaced `localStorage` as the source of truth for which shops a caller owns. **METIS 0.8.0 (2026-09-05, Milestone 10)** makes the backend deployable to a container host on a free tier with no credit card — Cloud Run is unreachable for this project because enabling it requires an international card even inside its free tier. Four blockers were fixed, three of them silent: the backend image had never built (its Dockerfile assumed a `backend/` build context while Cloud Build passed the repo root); neither Google SDK could receive credentials on a host with no metadata server, and `settings.GOOGLE_APPLICATION_CREDENTIALS` had never been read by any code, so setting it in `.env` had always done nothing; `get_db()` fell back to SQLite on any Firestore failure with only a `print`, which on an ephemeral container means serving happily and losing every order at the next restart; and `SettingsConfigDict` was passed a non-existent `env_delimiter` option, leaving `CORS_ORIGINS` JSON-only so the comma-separated value anyone would type into a hosting dashboard killed the container during import. `backend/src/core/credentials.py`, `METIS_REQUIRE_FIRESTORE`, a `/health` that reports the live database and auth mode, and `backend/Dockerfile` address these. Milestone 8 is no longer deprioritised: the project is now being shipped as a hosted product, which made ownership enforcement a prerequisite rather than optional hardening. Remaining work is the deployment itself, then commerce hardening and real-time updates.
...
| Environment configuration | ✅ Working | `backend/src/core/config.py` *(0.3.0: `APP_VERSION=0.3.0`; 0.4.1: `APP_VERSION=0.4.1` in config and `.env`; 0.4.3: `APP_VERSION=0.4.3`; 0.5.0: `APP_VERSION=0.5.0`; 0.6.0: `APP_VERSION=0.6.0`, adds `METIS_MOCK_AI`; 0.7.0: `APP_VERSION=0.7.0`)* |
...
| Endpoint | Status |
|----------|--------|
| `POST /api/business` | ✅ Implemented *(0.7.6: `require_user` — an anonymous caller may not mint a business nobody owns)* |
| `GET /api/businesses` | ✅ Implemented *(0.7.6: the caller's own shops; replaces `localStorage` as the source of truth. With auth off it returns everything, which is correct for a single-user local install)* |
...
| `GET /health` | ✅ Implemented *(0.8.0: reports the live database backend and whether auth is enforced, resolved from the real client rather than echoed from config)* |
...
| 18 | 0.7.0 | Testing | Committed pytest suite — `backend/tests/` (58 tests, isolated temp SQLite DB + mock AI per test via `conftest.py`): agent permissions, product/customer/order CRUD + ownership 404s, order booking/release/reapply on status change, revenue recognition, approval stage→approve/reject→execute (incl. `FAILED` on execution error), mock-AI chat tool dispatch, demo seeding | `backend/tests/`, `backend/pytest.ini` |
| 19 | 0.7.0 | Backend | Bug fix found while writing the above: `GET /analytics/{business_id}/revenue` ignored the documented `period` query param (`today`/`7d`/`30d`) — the route never declared it, so it always behaved like `all` | `backend/src/api/routes.py` |
...
| ~~No auth on any endpoint~~ | **Resolved (0.7.5-0.7.6).** Firebase identity + `require_business_access` on all 28 owner routes; the 3 storefront routes stay deliberately public. Off by default (`METIS_AUTH_ENABLED`) so local development is unchanged |

---

## 4. Missing Features (Per Milestones)

### Milestone 5 — Complete *(0.7.6)*
- **Authentication context** implemented — `frontend/src/lib/AuthContext.tsx`, a `/login` page (sign-in, signup, password reset), and a single guard in `frontend/src/components/AppShell.tsx` that all nine owner pages inherit through `(owner)/layout.tsx`
...
- `METIS_REQUIRE_FIRESTORE` turns the silent SQLite fallback into a startup failure; `/health` now reports the live database and whether auth is enforced
- Remaining: push to the Space, deploy to Vercel, confirm data survives a restart
...
1. ~~Complete Phase 1B demo experience~~ — mock AI mode, streaming chat, photo→product (vision), and voice briefing are all done as of 0.7.2; a scripted 90-second demo walkthrough is still open
...
### Priority 2 — Testing Foundation ✅ Done (0.7.0)
3. ~~Write unit tests for agent routing and permission enforcement~~ — `backend/tests/test_agent_permissions.py`
4. ~~Write integration tests for order creation and inventory updates~~ — `backend/tests/test_orders_api.py`, `test_approvals_api.py` (incl. the `ApprovalStatus.FAILED` path)
5. ~~Add test fixtures with mock Firestore and Gemini~~ — `backend/tests/conftest.py` (temp SQLite per test + `METIS_MOCK_AI=1`, reusing the existing local-fallback DB and mock-AI infra rather than building separate mocks)
