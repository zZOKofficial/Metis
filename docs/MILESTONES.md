...
**Last Updated:** 2026-09-05 (METIS 0.8.6)
...
- [x] Store-parity tests — every stored datetime is timezone-aware, so the SQLite used locally and the Firestore used in production behave identically, `backend/tests/test_datetime_awareness.py` *(0.8.6)*
...
> **0.8.6 (2026-09-05):** A bug that could only ever appear in production, found by running the app rather than the suite. Firestore returns every timestamp timezone-**aware** (`DatetimeWithNanoseconds`, UTC) no matter what was written; SQLite returns whatever was serialized, and the app wrote naive `datetime.utcnow()`. So every comparison between a stored timestamp and a freshly computed one passed locally and raised `TypeError: can't compare offset-naive and offset-aware datetimes` on the deployment. `GET /analytics/{business_id}/revenue?period=today|7d|30d` was a 500 on Render and a 200 everywhere it was tested — reachable only since 0.7.0, which is when `period` first started being passed through at all. Two quieter instances of the same mismatch: the chat-history sort fell back to a naive `datetime.min`, so a single message missing `created_at` would 500 the whole history; and the un-pushed-down datetime filter in `_matches()` caught the `TypeError` and returned `False`, meaning a datetime filter on Firestore silently matched **nothing** rather than failing loudly. `backend/src/core/clock.py` is now the single source of `now` — aware UTC — and `_revive()` promotes naive values from an existing SQLite file on read, so old rows migrate transparently instead of needing a backfill. The Pydantic schema defaults moved with it, which fixes a user-visible bug too: a naive datetime serializes with no offset, and the browser reads an offset-less ISO string as *local* time, so order timestamps rendered wrong by the viewer's UTC offset — six hours out in Dhaka. Verified across UTC, Asia/Dhaka and America/New_York. Tests: 151 → 160; the new file fails 8/9 against the old code. The 1006 deprecation warnings the suite used to emit are gone with it, since `datetime.utcnow()` is itself deprecated for exactly this reason.
